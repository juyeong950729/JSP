...
<%
	...
	// 글 가져오기
	ArticleVO art = dao.selectArticle(no);
	...
	$(document).ready(function(){
		...
		$(document).on('click', '.remove', function(e){
			...
			if (isDeleteOk){
				...
				$.ajax({
					url: '/FarmStory/proc/commentDeleteProc.jsp',
					type: 'GET',
					data: jsonData,
					dataType: 'json',
		...
		$(document).on('click', '.modify', function(e){
			...
			if(txt == '수정'){
				...
				$.ajax({
					url: '/FarmStory/proc/commentModifyProc.jsp',
					type: 'POST',
					data: jsonData,
					dataType: 'json',
					success: function(data){
		...
		$('.commentForm > form').submit(function(){
			...
			$.ajax({
				url:'/FarmStory/proc/commentWriteProc.jsp',
				method:'POST',
				data: jsonData,
				dataType:'json',
				success:function(data){
        ...
        <main id="board">
            <section class="view">
                <table border="0">
                    ...
                    <tr>
                        <th>제목</th>
                        <td><input type="text" name="title" value="<%= art.getTitle() %>" readonly/></td>
                    </tr>
                    <% if(art.getFile() > 0){ %>
                    <tr>
                        <th>파일</th>
                        <td><a href="/FarmStory/proc/download.jsp?parent=<%= art.getNo() %>"><%= art.getOriName() %></a>
                        &nbsp;<span><%= art.getDownload() %></span>회 다운로드</td>
                    </tr>
                    <% } %>
                    <tr>
                        <th>내용</th>
                        <td>
                            <textarea name="content" readonly><%= art.getContent() %></textarea>
                        </td>
                    </tr>                    
                </table>
                
                <div>
                	<% if(sessUser.getUid().equals(art.getUid())){ %>
                    <a href="/FarmStory/proc/deleteProc.jsp?no=<%= art.getNo() %>&pg=<%= pg %>&group=<%= group %>&cate=<%= cate %>" class="btn btnRemove">삭제</a>
                    <a href="./modify.jsp?group=<%= group %>&cate=<%= cate %>&no=<%= art.getNo() %>&pg=<%= pg %>" class="btn btnModify">수정</a>
                    <% } %>
                ...
                <section class="commentList">
					...
					<% for(ArticleVO comment : comments){ %>
                    <article>
